...
toolVar = document.querySelectorAll(".tool2"),
selectedTool1 = "monochromatic",
selectedTool2 = "none",
(function() {
    var ColorScheme,
      slice = [].slice;
...
const generatePalette1 = () => {
    ...
    let randomHex = Math.floor(Math.random() * 0xffffff).toString(16);
        ...
        if(selectedTool1 === "monochromatic") {
          MaxPaletteBoxes1 = 4;
          scheme.from_hex(randomHex)
            .scheme('tetrade')
           .distance(Math.random())
        }
        if(selectedTool1 === "contrast") {
          MaxPaletteBoxes1 = 8;
          scheme.from_hex(randomHex)
            .scheme('contrast')
           .distance(Math.random())
        }
        if(selectedTool1 === "triade") {
          MaxPaletteBoxes1 = 12;
          scheme.from_hex(randomHex)
            .scheme('triade')
           .distance(Math.random())
        }
        if(selectedTool1 === "tetrade") {
          MaxPaletteBoxes1 = 16;
          scheme.from_hex(randomHex)
            .scheme('tetrade')
           .distance(Math.random())
        }
        if(selectedTool1 === "analogic") {
          MaxPaletteBoxes1 = 12;
          scheme.from_hex(randomHex)
            .scheme('analogic')
           .distance(Math.random())
        }
        if(selectedTool2 === "none") {
        }
        if(selectedTool2 === "pastel") {
          scheme.variation('pastel');
        }
        if(selectedTool2 === "soft") {
          scheme.variation('soft');
        }
        if(selectedTool2 === "light") {
          scheme.variation('light');
        }
        if(selectedTool2 === "hard") {
    ...
    for (let i = 0; i < MaxPaletteBoxes1; i++) {
        ...
        color.innerHTML = `<div class="rect-box" style="background: ${"#"+colors[i]}"></div>
                           <span class="hex-value">${"#"+colors[i]}</span>`;
...
toolVar.forEach(btn => {
  btn.addEventListener("click", () => {
      document.querySelector(".options2 .active2").classList.remove("active2");
      btn.classList.add("active2")
      selectedTool2 = btn.id
      console.log(btn.id);
  })    
});